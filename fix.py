...
const LEVELS_F = ['All','Beginner','Intermediate','Advanced'];
...
export default function Partners() {
  ...
  const [sentRequests, setSentRequests] = useState(() => {
    try {
      const saved = localStorage.getItem('sc_sent_requests');
  ...
  useEffect(() => {
    api.get('/auth/users').then(res => {
      ...
      matchService.findPartners({}).then(res => {
        if (res && res.data && res.data.partners && res.data.partners.length) {
  ...
  const handleSend = (partnerId) => {
    setSentRequests(prev => {
      const updated = new Set([...prev, partnerId]);
      localStorage.setItem('sc_sent_requests', JSON.stringify([...updated]));
      return updated;
    });
  };
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal partner-modal" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", gap: "0.75rem", marginTop: "1.5rem" }}>
          <button className="auth-btn" style={{ flex: 1 }} onClick={onSend} disabled={sent}>
            {sent ? "Request Sent" : "Send Request"}
          </button>
          <button className="btn-outline-sm" style={{ flex: 1 }} onClick={onClose}>Maybe Later</button>
        </div>
      </div>
    </div>
  );
...
with open(
    r"C:\Users\DELL\CAREER\GLD\S4\APPLICATION NATIVE CLOUD\sportconnect-docker\frontend\src\pages\Partners.js",
